...
public class TestCritical extends Module {
    public TestCritical() {
        super("TestCritical", Category.COMBAT);
    }
    ...
    public void onPacketSend(PacketEvent.Send event) {
        if (event.getPacket() instanceof CPacketUseEntity) {
            ...
            if (packet.getAction() == CPacketUseEntity.Action.ATTACK && mc.player.onGround) {
                mc.player.connection.sendPacket(new CPacketPlayer.Position(mc.player.posX, mc.player.posY + .32767, mc.player.posZ, false));
