package com.Kometarou.OkomeClient.util.player;
...
public class CrystalUtil implements Util {
    public static NonNullList<BlockPos> placePostions(double placeRange, boolean opPlace) {
        NonNullList positions = NonNullList.create();
        positions.addAll(BlockUtil.getSphere(new BlockPos(Math.floor(mc.player.posX), Math.floor(mc.player.posY), Math.floor(mc.player.posZ)), (float) placeRange, (int) placeRange, false, true, 0).stream().filter(pos -> canPlaceCrystal(pos, true, opPlace)).collect(Collectors.toList()));
        return positions;
    }

    public static boolean canPlaceCrystal(BlockPos blockPos, boolean specialEntityCheck, boolean opplace) {
